...
"""
Synthesis + Report Writer Agent.

This node runs after all research agents complete (fan-in). It:
  1. Cross-references findings from all agents for consistency
  2. Identifies contradictions or gaps
  3. Writes a structured Markdown report with numbered citations
  4. Calculates total token usage and USD cost estimate
"""
...
# GPT-4o pricing as of 2026 (update when pricing changes)
...
async def synthesis_node(state: AgentState) -> dict:
    """
    Synthesis and Report Writer node.

    Runs after all parallel research agents complete. Aggregates all
    AgentFindings and produces the final structured Markdown report.
    """
    logger.info(
...
Write the research report now."""),
    ...
    # ── Aggregate metadata ────────────────────────────────────────────────────
    total_tokens = sum(f.get("tokens_used", 0) for f in findings)
    all_sources = []
    for finding in findings:
        all_sources.extend(finding.get("sources", []))

    # Deduplicate sources by URL (keep order of first appearance)
    ...
    synthesis_tokens = len(final_report) // 4
    total_tokens += synthesis_tokens
    cost_usd = (total_tokens / 1000) * _COST_PER_1K_INPUT_TOKENS
...
def _format_findings_for_llm(findings: list) -> str:
    """Format all agent findings into a readable block for the synthesis prompt."""
    sections = []
    for finding in findings:
